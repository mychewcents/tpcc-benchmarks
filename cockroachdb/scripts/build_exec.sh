...
echo "Example: ./setupCmd -env=prod -config=configs/prod/setup_5.json -node=1"

go build -o serverCmd cmd/server/*
echo "Example: ./serverCmd -env=prod -config=configs/prod/setup_5.json -node=1 (start | stop | init | load)"
...
echo "Example: ./dbstateCmd -env=prod -config=configs/prod/setup_5.json -node=1"
